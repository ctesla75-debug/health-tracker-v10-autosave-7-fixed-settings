...
// --- Persist measurement frequency settings (daily/weekly/custom) ---
const MEASURE_SETTINGS_KEY = "health_tracker_measure_settings_v1";

function saveMeasureSettings(){
  try{
    const modeEl = document.querySelector("#measureFreq");
    const ivEl = document.querySelector("#measureInterval");
    if(!modeEl) return;
    const mode = modeEl.value || "daily";
    const raw = Number(ivEl?.value);
    const intervalDays = Number.isFinite(raw) ? Math.max(1, Math.floor(raw)) : 7;
    const data = { mode, intervalDays };
    localStorage.setItem(MEASURE_SETTINGS_KEY, JSON.stringify(data));
  }catch(e){
...
function loadMeasureSettings(){
  try{
    ...
    const ivEl = document.querySelector("#measureInterval");
    if(!modeEl) return;
    const raw = localStorage.getItem(MEASURE_SETTINGS_KEY);
    if(!raw) return;
    const data = JSON.parse(raw);
    const mode = (data && typeof data.mode === "string") ? data.mode : null;
    if(mode && ["daily","weekly","custom"].includes(mode)){
      modeEl.value = mode;
    }
    if(ivEl){
      const intervalDays = Number.isFinite(Number(data?.intervalDays)) ? Math.max(1, Math.floor(Number(data.intervalDays))) : null;
      if(intervalDays) ivEl.value = String(intervalDays);
    }
  }catch(e){
    // ignore
  }
...
async function measurementsAllowed(dateISO){
  ...
  // Anchor = earliest stored log date (first day you ever saved anything)
  const all = await getAllLogs();
  if(!all.length) return true;

  const anchorISO = all.map(l=>l?.date).filter(Boolean).sort()[0];
  ...
  const a = toUtcDay(anchorISO);
  const s = toUtcDay(dateISO);
  if(a===null || s===null) return true;

  const diff = s - a; // days
  // Allow any date that lands exactly on the interval from the anchor
...
window.addEventListener("load", async ()=>{
  ...
  // Restore saved measurement frequency settings
  loadMeasureSettings();
  ...
  document.querySelector("#measureInterval").addEventListener("input", ()=> { saveMeasureSettings(); setSaveEnabled(true); });
